...
class sfValidatorFile extends sfValidatorBase
{
  ...
  protected function configure($options = array(), $messages = array())
  {
    ...
    //Opciones agregadas para la calidacion de tamaño by Miguel
    $this->addOption('ancho', null);
    $this->addOption('alto', null);
    ...
    $this->addMessage('tamano', 'Las dimensiones deben ser 800x415.');
  }
  ...
  protected function doClean($value)
  {
    ...
    // CODIGO AGREGADO PARA VALIDAR DIMENSIONES DE IMAGENES
    if($this->hasOption('ancho') && $this->hasOption('alto')){
      $size = getimagesize($value['tmp_name']);
      $ancho = $size[0];
      $alto = $size[1];
      
      if($ancho != $this->getOption('ancho') && $alto != $this->getOption('alto')){
        throw new sfValidatorError($this, 'tamano');
      }
    }
